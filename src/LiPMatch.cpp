...
void LiPMatch::localize(tools::m_keyframe &c_keyframe, int keyFrameCount)
{
    ...
    std::map<unsigned, unsigned> resultingMatchtvt;
    resultingMatchtvt.clear();
    ...
    std::map<unsigned, unsigned> resultingMatchtvtc;
    resultingMatchtvtc.clear();
    ...
    std::vector<std::map<unsigned, unsigned>> AllbestPlaneMatch;
    AllbestPlaneMatch.clear();
    std::vector<std::map<unsigned, unsigned>> AllbestVehicleMatch;
    AllbestVehicleMatch.clear();
    std::vector<std::map<unsigned, unsigned>> AllbestCylinderMatch;
    AllbestCylinderMatch.clear();

    std::vector<int> AllSamebestMatchSize;
    AllSamebestMatchSize.clear();
...
void LiPMatch::loopClosure(tools::m_keyframe &c_keyframe, int keyFrameCount)
{
    std::cout << "new keyframe coming... " << keyFrameCount << std::endl;
    static pcl::VoxelGrid<pcl::PointXYZI> grid_1;
    grid_1.setLeafSize(0.40, 0.40, 0.40);
    ...
    Eigen::Vector3d t_curr = c_keyframe.m_pose_t;
    ...
    if (last_keyframe->vPlanes.size() > 2)
    {
        ...
        for (size_t his = 0; his < pointSearchInd.size(); his++)
        {
            ...
            Subgraph currentSubgraph(keyframe_vec[pointSearchInd[his]]->vPlanes);
            Subgraph targetSubgraph(last_keyframe->vPlanes);
    ...
    if (last_keyframe->vVehicles.size() > 2)
    {
        ...
        for (size_t his = 0; his < pointSearchInd.size(); his++)
        {
            ...
            Subgraph currentSubgraph(keyframe_vec[pointSearchInd[his]]->vVehicles);
            Subgraph targetSubgraph(last_keyframe->vVehicles);
    ...
    if (last_keyframe->vPoles.size() > 2)
    {
        ...
        for (size_t his = 0; his < pointSearchInd.size(); his++)
        {
            ...
            Subgraph currentSubgraph(keyframe_vec[pointSearchInd[his]]->vPoles);
            Subgraph targetSubgraph(last_keyframe->vPoles);
    ...
    std::cout << "find loop-closure successful!" << std::endl;
}
...
void LiPMatch::run()
{
    ...
    while (!LiPMatch_stop) // Stop loop if LiPMatch
    {
        ...
        {
            if (isLocalize)
                localize(frameQueue[numPrevKFs], numPrevKFs);
            else
            {
                loopClosure(frameQueue[numPrevKFs], numPrevKFs);
            }
